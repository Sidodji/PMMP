...
int main(int argc, const char** argv)
{
    CommandLineParser parser(argc, argv,
        ...
        "{mouth_cascade|data/haarcascades/haarcascade_smile.xml|Path to mouth cascade.}"
...
void detectAndDisplay(Mat frame)
{
    ...
    for (size_t i = 0; i < faces.size(); i++)
    {
        ...
        {  
            Point center(faces[i].x + faces[i].width/2, (faces[i].y + faces[i].height)+20);
            putText(frame, "Smiling", center,
                FONT_HERSHEY_PLAIN, 2, (255, 0, 0), 2, LINE_AA);
